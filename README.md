...
当然，也有一部分通信是利用 Vuex 来管理的，基本的使用请参考[官网](https://vuex.vuejs.org/zh-cn/)和本项目的 **store** 文件夹，这里主要讲一下使用原则。一般来说，如果是一个在诸多组件中都会产生影响的变量，而且这些组件相互之间不是简单的父子关系，我们才会用到 Vuex，无节制地使用它会造成没必要的复杂性，但是合理利用 Vuex 却能帮忙解决很多困难。例如，用户的显示权限，是脱敏还是全显，是一个全局状态，不依附某个具体的组件，而是要影响到诸多组件的显示状态，因此用 Vuex 来管理是合适的。

值得一提的是，结合路由信息对象，以及 Vue 的计算型属性，可以更好地发挥 Vuex 的作用。例如我在 **ListType.vue** 组件中申明的 `listType`，就相当于把路由信息映射成了全局 Vuex 变量，虽然路由信息本身也是全局的（this.$route 和 this.$store.state 在功能上是有些类似的），但是这样强制指定了映射关系之后，大大方便了以后的调用，让代码简洁许多（`listType` 会影响到诊断模块的权限控制，它本身是由路由决定的，但是如果你在每个需要该信息的地方，都去重新判断一遍路由对象，会产生大量的重复代码）。
