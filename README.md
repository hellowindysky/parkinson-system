...
## 依赖库打包
由于项目的依赖库并不会频繁变更，因此每次打包时都对它们进行打包既浪费时间又没有必要。更重要的是，由于每次打包出来的js文件名含有hash值，因此都不一样，用户在浏览器端也就没法对文件进行缓存。如果把依赖库完全用CDN的方式在 `script` 标签中引入（结合 webpack 的 **externals** 配置项），对于某些依赖关系复杂的库，还是会出现重复打包的问题。因此，这里使用了 DLLPlugin 和 DLLReferencePlugin 这两个 webpack 插件来解决这个问题。基本思想就是提前将不易变的依赖库打包成一个静态文件 **static/js/vendor.dll.js**，同时生成配置文件 **build/vendor-manifest.json**，以后在打包的过程中就不会对这些依赖库重复打包，缩减了打包时间。而且，生成的 **vendor.dll.js** 文件可以在用户浏览器端进行缓存。
