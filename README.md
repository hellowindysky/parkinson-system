...
# 初次打包之前，以及相关依赖库发生变化后，都务必执行下面这行命令，详情参考后面的“依赖库打包”
npm run dll
...
## 依赖库打包
由于项目的依赖库并不会频繁变更，因此每次打包时都对它们进行打包既浪费时间又没有必要。更重要的是，由于每次打包出来的js文件名含有hash值，因此都不一样，用户在浏览器端也就没法对文件进行缓存。如果把依赖库完全用CDN的方式在 `script` 标签中引入（结合 webpack 的 **externals** 配置项），对于某些依赖关系复杂的库，还是会出现重复打包的问题。因此，这里使用了 DLLPlugin 和 DLLReferencePlugin 这两个 webpack 插件来解决这个问题。基本思想就是提前将不易变的依赖库打包成一个静态文件 **static/js/vendor.dll.js**，同时生成配置文件 **build/vendor-manifest.json**，以后在打包的过程中就不会对这些依赖库重复打包，缩减了打包时间。而且，生成的 **vendor.dll.js** 文件可以在用户浏览器端进行缓存。注意，这个文件需要在 **index.html** 文件中用 `script` 标签引入，而且最好早于其它 js 文件之前。

因此，在初次打包前，以及依赖库发生变化后的下一次打包之前，都需要重新生成这个 **vendor.dll.js** 文件，即执行以下命令：
``` bash
npm run dll
```
...
## 依赖库
到我写下这段话之前，整个项目的依赖库都是通过包管理工具来进行整理的，也就是说它们全部装在了 node_modules 文件夹里(甚至包括 **normalize.css** 这样的单文件)，通过 `npm install` 命令就可以把所有需要的库全部下载。我尽量避免去使用 script 标签直接引入线上库，也没有像其它项目那样把一些库的代码下载到本地后再 import 使用，一方面是我希望做到用法统一（在需要引入库文件的组件内使用 import 命令），另一方面在于这样易于管理（全部通过 **package.json** 文件来管理）。
...
> 2018年2月份新增以下内容：我曾经以为自己搞清楚了 package-lock.json 的工作原理，但是现在意识到其实不是这样的。这个文件和 package.json 之间的关系随着 npm 的版本升级而历经了几次变化，网上相关讨论很多，甚至官方文档在某个时刻也是存在误导性的。这个问题也许会在某刻时刻得到彻底解决，但完全搞清楚之前（那个时候可以来更新这一段文档），还是多看看网上的资料吧，这里是一个信息量比较大的链接 [#17979](https://github.com/npm/npm/issues/17979)。再次重申，如果你只是读了网上的几篇文章，就认为自己搞清楚了，那么你很有可能搞错了，npm 的这一块设计本身就引来了很多吐槽，修改这一段文档之前务必自己做足本地测试。

> #### 关于 jquery
> 这个著名的插件和它所代表的开发模式也值得在这里说明一下，由于 Vue 的组件开发是建立在数据驱动或者说状态驱动的思想上的，因此并不提倡去操作 DOM 元素，我迄今也没有在任何组件里面 import 这个插件。偶尔避免不了要去操作单个 DOM 元素时，Vue 也提供了 refs 这个属性供我们使用。如果日后因为某些库要使用到 jquery ，可能还是得重新让它回归，但目前我们还是遵循 Vue 的方式，不要和 jquery 打交道吧。
...
## 命名规范
项目的命名遵循统一的准则：
1. 由于 linux 系统对文件路径的大小写不敏感（git的版本控制默认也是如此），因此文件路径统一采用全小写（除了一个例外，在下面第2点）。你会发现，组件的路径采用了全名，由于单词很长而且是全小写，导致一长串字母很难阅读，这是一个待修改的地方，单词之间应该用短横线或者下划线隔开。
2. vue的单个组件采用大写驼峰式写法（即首字母大写的驼峰式命名规范，下同），这个不是我们定的，用 vue-cli 工具初始化项目的时候，自动生成的组件就叫 HelloWorld.vue，因此项目对待 vue 组件也遵循该规则。
3. 绝大部分 js 变量都遵循小写驼峰式的写法，除了一些构造函数（例如 Stomp）或者单例对象（例如 Bus）采用首字母大写。
4. 所有 css 类名，less 变量名和 html 标签名都采用纯小写加短横线隔开的写法。因此，在一个 vue 实例的 components 属性中引入的 js 变量（代表引入的组件名）虽然采用小写驼峰法（如 aSimpleComponent），但是真正写到 html 里面的，依然是小写加短横线的形式（如 a-simple-component）。
5. 常量统一采用全大写，并用下划线隔开。
...
## 权限管理
本项目作为一个后台管理系统，涉及到不同的用户角色（医生／录入员／管理员），不同的患者列表类型（医生／下属医生／项目医生／治疗者／评估者），不同的入口（医院／课题），不同的显示模式（脱敏／全显），不同的患者阶段（双盲实验期间的各个不同阶段），这些会影响到组件的渲染，操作权限的变化，以及路由的变更。除了由后台返回所需要的变量之外，大部分的权限管理工作由前端来完成，具体的实现规则无法细讲，这里只说几个总体原则：

1. 对于刷新页面后需要继续保留的信息（例如系统默认是脱敏显示，而当前用户状态是全显模式，那么刷新页面后由于js代码重置，这个显示状态就必须依赖于js之外的地方来进行保存），其所依赖的根变量必须存在于浏览器的 sessionStorage 或 localStorage，前者生命周期同本次浏览器会话（当前标签页），更符合我们的需要，因此在项目中使用得更多一些。
2. 上一条中存储的内容在浏览器中是可以被手动修改的，而且对用户可见，因此只在必要时使用。代码中使用到的一些权限变量，如果是全局的，尽量使用 **vuex** 来进行管理，它们对用户不可见也不能被修改，而且其变更操作也更容易集中管理。注意有些 **vuex** 全局变量是依赖 sessionStorage 中的值来变化的。
3. 这里特别说一下路由地址的问题，有些组件的渲染与否，以及组件内部的权限，是依赖于路由的。也就是说，路由地址是我们这里的根变量。事实上，路由信息对象 `route` 本身就是一个全局变量，在我看来，它和 **vuex** 所管理全局变量的区别无非在于，它的变更由地址栏的变化所触发，而后者由 `COMMIT` 操作来直接触发，两者的使用思路有相似之处。另外，为了避免用户随意更改路由，可以配置 **vue-router** 的(导航守卫)[https://router.vuejs.org/zh-cn/advanced/navigation-guards.html]来控制路由的权限。
4. 另外强调一下，灵活使用 `props` 数据传递，如果父子组件具有相同的权限控制，或者它们的权限具有继承关系，那么就可以把权限当作变量传递下去，减少冗余代码。

总之，项目的实际情况是复杂而多变的，以上是基本的思路，灵活使用即可。
